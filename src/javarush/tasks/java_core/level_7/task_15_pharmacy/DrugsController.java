...
public class DrugsController {
	...
	public synchronized void buy(Drug drug, int count) {
		...
		if(currentCount < count) {
			System.out.printf("%s хочет %s %d штук. В наличии %d\n ", name, drug.getName(), 
					count, currentCount);
		}else {
			allDrugs.put(drug, (currentCount - count));
			System.out.printf("%s купила %s %d штук. Осталось %d\n", name, drug.getName(), 
					count, (currentCount - count));
		}
	}
	
	public synchronized void sell(Drug drug, int count) {
		System.out.println(Thread.currentThread().getName() + " Закупка " + drug.getName() +
				" " + count);
